...
- Phase：已完成后端双引用、前端语义字段与查询失效；准备真实桌面回归。
- Acceptance：文本/PDF/Word/Excel 聊天上传后在文档目录可见、索引状态实时更新；同一文件在会话附件仍可引用；媒体和不支持格式行为不变。
- Blocker：真实桌面验收尚未执行。

验证结果：定向附件回归 17/17、全量回归 51/51、`pnpm run typecheck`、改动文件 Biome lint、`git diff --check` 通过；Mastra `/health` 返回 200。未重打包，开发服务未停止；下一步由用户在桌面端上传文本/PDF 并确认“文档目录”和“会话附件”两处均符合预期。
