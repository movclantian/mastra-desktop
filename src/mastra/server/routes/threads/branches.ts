 ...
 * 物理删除不丢可切换的历史。
 */
async function deletePersistedTail(memory: Memory, messages: UIMessage[], startIndex: number) {
...
export async function prepareMessageBranchOperation(options: {
  ...
  if (options.trigger === "regenerate-message") {
    ...
    await deletePersistedTail(options.memory, messages, index);
    return {
      kind: "regenerate",
...
async function prepareInjectedRegenerate(
  ...
  await deletePersistedTail(options.memory, messages, parentIndex + 1);
  return {
    kind: "regenerate",
    messageId,
