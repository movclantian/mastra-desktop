      ...
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        ...
        >
          <SheetHeader className="sr-only">
            <SheetTitle>{i18n.t("sidebar:navigation")}</SheetTitle>
            <SheetDescription>{i18n.t("sidebar:displaysMobileSidebar")}</SheetDescription>
          </SheetHeader>
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    ...
    >
      <PanelLeftIcon />
      <span className="sr-only">{i18n.t("sidebar:toggleSidebar")}</span>
    </Button>
  );
}
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "relative ml-3.5 flex min-w-0 translate-x-px flex-col gap-1 pl-2.5 pr-0 py-0.5 before:pointer-events-none before:absolute before:inset-y-0 before:left-0 before:w-px before:bg-sidebar-border group-data-[collapsible=icon]:hidden",
        className,
      )}
      {...props}
    />
...
function SidebarMenuSubButton({
  ...
  return useRender({
    ...
    props: mergeProps<"a">(
      {
        className: cn(
          "flex h-7 w-full min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground ring-sidebar-ring outline-hidden group-has-data-[sidebar=menu-action]/menu-sub-item:pr-7 group-data-[collapsible=icon]:hidden hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[size=md]:text-sm data-[size=sm]:text-xs data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-accent-foreground",
          className,
        ),
      },
      props,
