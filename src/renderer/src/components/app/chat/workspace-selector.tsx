...
import { dirName, type RecentWorkspace, useWorkbench } from "@/lib/workbench";

// ---------------------------------------------------------------------------
// 工作区选择器(输入框上方的独立卡片,新会话首条消息前显示):
// 下拉列出近期绑定过的目录,或调用系统目录选择器选定本地工作目录;
// 随首条消息以 body.workspacePath 上传,服务端锁定到 thread.metadata
// 后本组件即不再渲染(会话工作区不可中途更换)。
// 卡片去底边(rounded-b-none border-b-0)与下方输入框无缝相接;
