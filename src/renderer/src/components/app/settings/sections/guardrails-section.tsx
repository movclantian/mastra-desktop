...
export function GuardrailsSection() {
  ...
  const refreshStatus = React.useCallback(() => {
    fetch(`${MASTRA_SERVER_URL}/work/guardrails/status`)
      .then((r) => (r.ok ? r.json() : null))
      .then((next: GuardrailsStatus | null) => {
  ...
  React.useEffect(() => {
    if (loaded) return;
    fetch(`${MASTRA_SERVER_URL}/work/guardrails`)
      .then((r) => (r.ok ? r.json() : null))
      .then((config: Partial<GuardrailsDraft> | null) => {
        if (!config) return;
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch(`${MASTRA_SERVER_URL}/work/guardrails`, {
        method: "POST",
