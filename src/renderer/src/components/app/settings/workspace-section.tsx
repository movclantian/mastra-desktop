...
import { SettingCard, SettingRow, SliderRow } from "./shared";
...
export function WorkspaceSection() {
  ...
  React.useEffect(() => {
    if (loaded) return;
    fetch(`${MASTRA_SERVER_URL}/work/workspace`)
      .then((r) => (r.ok ? r.json() : null))
      .then((config) => {
        if (config) setDraft((prev) => ({ ...prev, ...config }) as WorkspaceDraft);
      })
      .catch(() => undefined)
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch(`${MASTRA_SERVER_URL}/work/workspace`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...draft,
          sandboxEnv: Object.fromEntries(
            Object.entries(draft.sandboxEnv).filter(([k]) => k.trim()),
          ),
      ...
      >
        <div className="space-y-2 py-4">
          <div className="space-y-1">
            <p className="text-sm leading-none font-medium">线程工作区根目录(threadsRoot)</p>
