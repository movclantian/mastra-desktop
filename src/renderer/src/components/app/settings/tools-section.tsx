...
export function ToolsSection() {
  ...
  React.useEffect(() => {
    if (loaded) return;
    fetch(`${MASTRA_SERVER_URL}/work/tools`)
      .then((r) => (r.ok ? r.json() : null))
      .then((config: ToolsConfig | null) => {
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void fetch(`${MASTRA_SERVER_URL}/work/tools`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          tavily: { apiKey: draft.tavily.apiKey.trim() },
          firecrawl: {
            apiKey: draft.firecrawl.apiKey.trim(),
            apiUrl: draft.firecrawl.apiUrl.trim(),
          },
          anysearch: { apiKey: draft.anysearch.apiKey.trim() },
        }),
      })
