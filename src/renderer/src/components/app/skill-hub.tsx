...
import { McpDialog, type McpFormServer } from "@/components/app/mcp-dialog";
    ...
    <section className="mt-5">
      ...
      <div className="mt-2 grid gap-x-12 md:grid-cols-2">
        {skills.map((skill, index) => {
          ...
          return (
            <div className="flex min-w-0 items-center gap-3 border-b py-4" key={skill.name}>
              <button
  ...
  return (
    <div className="mt-8">
      ...
      <div className="grid gap-x-12 md:grid-cols-2">
        {skills.map((skill, index) => (
          <button
            className="flex min-w-0 items-center gap-3 border-b py-4 text-left"
            key={skill.name}
            onClick={() => onSelect(skill)}
            type="button"
          >
