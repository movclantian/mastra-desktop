  ...
  const uploadFiles = async (files: File[], target: LibraryUploadTarget) => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const session = await createLibraryUploadSession(resourceId, {
          filename: file.name,
        ...
        await completeLibraryUpload(session.id, resourceId);
  ...
  const cancelUpload = async () => {
    ...
    await Promise.all(
      sessions.map(async ([sessionId, resumeKey]) => {
        localStorage.removeItem(resumeKey);
        await cancelLibraryUpload(sessionId, resourceId).catch(() => undefined);
      }),
    );
  };
