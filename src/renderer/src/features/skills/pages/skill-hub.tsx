...
import { McpDialog } from "@/features/integrations";
import { type SkillMetadata, useWorkbench } from "@/features/workbench";
import { toastError } from "@/lib/errors";
import { cn } from "@/lib/utils";
import { deleteSkillMarketplace, saveSkillMarketplace } from "../api";
import { useSkillActions } from "../hooks/use-skill-actions";
import { useSkillHubData } from "../hooks/use-skill-hub-data";
import { formatInstalls, getPaginationRange, skillIcon, skillSourceLabel } from "../lib/display";
import type {
  LeaderboardView,
  MarketCategory,
  McpSummary,
      ...
      <ScrollArea className="min-h-0 flex-1" ref={scrollAreaRef}>
        <main className="mx-auto w-full max-w-6xl px-5 py-8">
          ...
          <div className="mt-6 flex items-center justify-between border-b pb-1">
            <Tabs
              onValueChange={(value) => setSection(value as SkillSection)}
...
function MarketplacesDialog({
  ...
  const save = async () => {
    setSaving(true);
    try {
      await saveSkillMarketplace({ id: editing?.id, name, url, branch, enabled: true });
