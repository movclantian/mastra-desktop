...
export function MemorySection() {
  ...
  React.useEffect(() => {
    ...
    fetchThreadObservationalMemory<{ config?: Record<string, unknown> }>(activeThreadId, user.id)
      .then((payload) => {
  ...
  React.useEffect(() => {
    ...
    setThreadSubagentLoaded(false);
    setThreadSubagentModels({});
    fetchThreadSubagentModels<{ subagentModels?: Record<string, string> }>(
      activeThreadId,
      user.id,
      controller.signal,
    )
  ...
  const saveThreadOmConfig = async () => {
    if (!activeThreadId) return;
    try {
      await saveThreadObservationalMemory(activeThreadId, user.id, {
        observation: {
      ...
      toast.success("当前线程的观察记忆覆盖已保存");
    } catch {
      toast.error("当前线程的观察记忆覆盖保存失败");
    }
  };
              ...
              <div className="w-64">
                <ModelSelectDropdown
                  ...
                  onChange={(modelId) => {
                    ...
                    void saveThreadSubagentModel(
                      activeThreadId,
                      user.id,
                      agentType,
                      modelId === FOLLOW_CURRENT_MODEL ? null : modelId,
                    ).catch(() => {
